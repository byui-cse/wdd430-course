---
title: SleepOutside Team 1
description: This activity will introduce the Sleepoutside Team project that we will work on over much of the semester.
time: 2 hours
---
...
It will be very helpful if you have completed the Github course: [Introduction to Github](https://github.com/skills/introduction-to-github) before you do this.

If your group is ready to start the activity and you have not had a chance to complete the course above...you can quickly review this [shorter GitHub guide](https://guides.github.com/activities/hello-world/) or this [guide specifically about using git with VS Code](https://code.visualstudio.com/docs/editor/versioncontrol) (Just remember to go back and do the other...it goes into much more detail).

## Core Requirements

### **01** Getting Started
  *   Each member of the team should have already completed the [course setup](../../resources/course-setup/) activity. Go and do that now if you have not.
    ...
    *   The driver for this week should then log into GitHub, and visit this [repository](https://github.com/matkat99/sleepoutsideClient) for the project. Click on the "Use this template" button to create a copy of the base project code in a new repository then give the new repo a name.
    *   The driver should then go to Settings->Manage Access and add the rest of the team members as collaborators. (Each team member needs to have their own GitHub account for this step. Create one if you do not have one.)
    *   Each member of the team should then clone the repository to their computer.

### **02**  Finish the project setup:

1. Open the project in VS Code. File->Open, then select the *directory* that the repository cloned into.
...
3. Notice that a new directory was created: `node_modules`
4. This project comes with several tools built in to help the team to write error free consistent code. The driver should open up the `package.json` file. There are two sections initially you should review. `devDependencies`, and `scripts`.
    ...
    *   `format`: This will run Prettier on your code and format it all consistently. You should also do this as a step in your commit process.
    ...
    >If you are using VS Code there are extensions you can install that will format (Prettier) and lint (ESLint) your code everytime you save! Alternatively you could take advantage of npm and scripts such as [prebuild](https://docs.npmjs.com/cli/v6/using-npm/scripts) to further automate these tasks

5. Each person in the group should try running `npm run start`. This runs several processes on our code and launches a web server at `localhost:8080`. It should also open that address in your default browser as well, if it did not you can type that address directly into a browser. You should use this server as you develop your project instead of something like LiveServer!
...
6. The driver should run `npm run build`. This will create a directory called `dist`. Commit those changes and push them to GitHub. If you go to github.com you will notice that the `dist` directory did not transfer with our commit. If you check the `.gitignore` file that is part of your project you will see why.
7. We will be using a product called [Netlify](https://www.netlify.com/) to host our site. One person in the group will need to create a free account there and login for the next step. **Note!** The person who created the repo for the project should be the one that does the setup in Netlify as well.
8. After logging in you need to add a new site. Click on the "Add New Site" button and select "Import an Existing Project". Connect to your Github account using the "Github" button. You should see a list of repositories on your account. Find the one that contains the Sleepoutside project.
9. Select the "main" branch. Then in the "build command" box enter `npm run build`. Then "Deploy Site". Netlify will produce a randomized URL for the site that should be shared with all the team members. This is the link to your "production" site. (You could even put this into the `Readme.md` file to make it easy to find!)
...
### **03** GitHub with a team

    1.  The driver should create a file in the root of the project called `team.txt` and add the link to the team Trello board, and their name. Then commit that change, and push it up to GitHub.
    2.  Each other member of the team should pull the changes down and add their names. Then commit and push.
    3.  The driver should then edit `team.txt`. Add your middle initial. Then commit and push. What happened?
        ...
        >*   Take advantage of software design techniques like [modules](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Modules) to break your code up into smaller pieces.
        >*   Finally take advantage of Git's ability to *branch*. See the stretch goals for more on that.

    4.  Before you can commit the merge conflict will need to be resolved. Make sure that the `team.txt` file is open. You should see some additions to the file from Git. It should look something like below.

        ![merge error from GitHub](/images/330-merge-conflict.PNG)
...
### **01**  Branching
    ...
    1.  Create a branch. Name your branch something like `initials--branch-test`. (So for me the actual name of the branch would be `st--branch-test`)
    2.  Add a file in the root of the project called `yourname.txt`. Type your name in the file.
    3.  Commit and publish (push) your branch to GitHub
    4.  Push your commit to GitHub...then do a pull after the other members of the team have all pushed their branches.
    5.  Switch to one of the new branches in the repository. Notice what happens to the listing of files in your editor when you do. Switch back to your own branch. Switch back to the master branch. Discuss with your team what you saw. In VSCode there are two ways to switch branches. First, if you use the command line you can run `git checkout branch-name`. The other way is to look at the bottom left corner of the VSCode window. You will see the current branch listed. Click on it to switch branches. If a branch you are expecting does not show up try doing a `git fetch`.
...
### **02**  Pull requests
    ...
    After the pull request has been made (you can do this through [github.com](https://guides.github.com/activities/hello-world/#pr)) the driver should go to the repository in GitHub and review the request and then finish the merge. Make sure that any extra branches are deleted once you are done merging.
